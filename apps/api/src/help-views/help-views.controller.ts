...
export class HelpViewsController {
  ...
  async getHelpViewState(@Req() req, @Param('pageKey') pageKey: string) {
    return this.helpViewsService.getHelpViewState(req.user.id, pageKey);
  }
  ...
  async incrementViewCount(@Req() req, @Param('pageKey') pageKey: string) {
    return this.helpViewsService.incrementViewCount(req.user.id, pageKey);
  }
  ...
  async resetAllHelpViews(@Req() req) {
    return this.helpViewsService.resetAllHelpViews(req.user.id);
  }
  ...
  async getAllHelpViewStates(@Req() req) {
    return this.helpViewsService.getAllHelpViewStates(req.user.id);
  }
}
