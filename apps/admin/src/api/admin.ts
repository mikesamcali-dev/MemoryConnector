...
export async function updateUserRoles(userId: string, roles: string[], token: string): Promise<User> {
  const headers = new Headers();
  headers.set('Authorization', `Bearer ${token}`);
  headers.set('Content-Type', 'application/json');
...
export async function updateUserTier(userId: string, tier: 'free' | 'premium', token: string): Promise<User> {
  const headers = new Headers();
  headers.set('Authorization', `Bearer ${token}`);
  headers.set('Content-Type', 'application/json');
