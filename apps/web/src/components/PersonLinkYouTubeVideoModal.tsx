...
import { linkYouTubeVideoToPerson } from '../api/people';

interface PersonLinkYouTubeVideoModalProps {
  personId: string;
  personName: string;
  isOpen: boolean;
  onClose: () => void;
...
export function PersonLinkYouTubeVideoModal({
  personId,
  personName,
  isOpen,
  onClose,
  linkedYouTubeVideoIds,
}: PersonLinkYouTubeVideoModalProps) {
  ...
  const linkMutation = useMutation({
    mutationFn: (youtubeVideoId: string) => linkYouTubeVideoToPerson(personId, youtubeVideoId),
    onSuccess: () => {
        ...
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              ...
              Link YouTube Videos to Person
            </h2>
            <p className="text-sm text-gray-600 mt-1">
              Add YouTube videos to "{personName}"
            </p>
          </div>
          <button
            onClick={onClose}
