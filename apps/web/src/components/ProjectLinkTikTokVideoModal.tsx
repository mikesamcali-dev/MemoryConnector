...
interface ProjectLinkTikTokVideoModalProps {
  projectId: string;
  projectName: string;
  isOpen: boolean;
  onClose: () => void;
...
export function ProjectLinkTikTokVideoModal({
  projectId,
  projectName,
  isOpen,
  onClose,
  linkedTikTokVideoIds,
}: ProjectLinkTikTokVideoModalProps) {
  ...
  const linkMutation = useMutation({
    mutationFn: (tiktokVideoId: string) => linkTikTokVideoToProject(projectId, tiktokVideoId),
    onSuccess: () => {
        ...
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              ...
              Link TikTok Videos to Project
            </h2>
            <p className="text-sm text-gray-600 mt-1">
              Add TikTok videos to "{projectName}"
            </p>
          </div>
          <button
            onClick={onClose}
