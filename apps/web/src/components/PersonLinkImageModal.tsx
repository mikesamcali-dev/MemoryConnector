...
import { linkImageToPerson } from '../api/people';

interface PersonLinkImageModalProps {
  personId: string;
  personName: string;
  isOpen: boolean;
  onClose: () => void;
...
export function PersonLinkImageModal({
  personId,
  personName,
  isOpen,
  onClose,
  linkedImageIds,
}: PersonLinkImageModalProps) {
  ...
  const linkMutation = useMutation({
    mutationFn: (imageId: string) => linkImageToPerson(personId, imageId),
    onSuccess: () => {
        ...
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              ...
              Link Images to Person
            </h2>
            <p className="text-sm text-gray-600 mt-1">
              Add images to "{personName}"
            </p>
          </div>
          <button
            onClick={onClose}
