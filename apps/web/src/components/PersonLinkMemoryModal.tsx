...
import { linkMemoryToPerson } from '../api/people';

interface PersonLinkMemoryModalProps {
  personId: string;
  personName: string;
  isOpen: boolean;
  onClose: () => void;
...
export function PersonLinkMemoryModal({
  personId,
  personName,
  isOpen,
  onClose,
  linkedMemoryIds,
}: PersonLinkMemoryModalProps) {
  ...
  const linkMutation = useMutation({
    mutationFn: (memoryId: string) => linkMemoryToPerson(personId, memoryId),
    onSuccess: () => {
        ...
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              ...
              Link Memories to Person
            </h2>
            <p className="text-sm text-gray-600 mt-1">
              Add memories to "{personName}"
            </p>
          </div>
          <button
            onClick={onClose}
